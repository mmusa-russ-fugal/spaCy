...
{/* BLOCKLY-PIPELINE-BUILDER: preset "pipelines-overview" (src/widgets/blockly/presets.ts).
Primary home of the widget, full "build" mode: toolbox with all built-in
component factories, per-component config fields, disable toggles and sourced
components. Generates spacy.blank()/spacy.load() + nlp.add_pipe() Python
matching the #built-in, #disabling and #sourced-components sections below. */}
