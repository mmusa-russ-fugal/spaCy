...
{/* BLOCKLY-PIPELINE-BUILDER: preset "pipelines-overview" (src/widgets/blockly/presets.js).
Primary home of the widget, full "build" mode: toolbox with all built-in
component factories plus a generic custom component block, placement arguments
(before/after/first/last), disable/exclude toggles and sourced components.
Generates spacy.blank()/spacy.load() + nlp.add_pipe() Python matching the
#built-in, #disabling and #sourced-components sections below. */}

<BlocklyPipelineBuilder preset="pipelines-overview" title="Try it: build a pipeline" />
