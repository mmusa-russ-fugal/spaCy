...
{/* BLOCKLY-PIPELINE-BUILDER: preset "spacy-101" (src/widgets/blockly/presets.ts).
Compact locked-down "tour" mode for first-time readers: pre-loaded with the
en_core_web_sm pipeline shown in the diagram above, no toolbox, Python output
pane. Links to /usage/processing-pipelines for the fully editable builder. */}

<BlocklyPipelineBuilder preset="spacy-101" title="Try it: explore a processing pipeline" />
