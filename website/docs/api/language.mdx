...
mutually-exclusive placement arguments (before/after/first/last) plus optional
name=, source= and config=, generating the corresponding nlp.add_pipe() call.
Shows an inline error hint when more than one placement argument is set,
mirroring the runtime validation described above. */}
