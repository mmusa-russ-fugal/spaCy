...
{/* BLOCKLY-PIPELINE-BUILDER: preset "api-add-pipe" (src/widgets/blockly/presets.js).
Minimal "snippet" mode: a single component block with fields for the
placement argument (before/after/first/last, single select) plus optional
name= and source=, generating the corresponding nlp.add_pipe() call.
Placement being a single select means the multi-argument conflict described
above can't be produced here. TODO: config= field, once the real editor
lands. */}

<BlocklyPipelineBuilder preset="api-add-pipe" title="Try it: build an add_pipe call" />
